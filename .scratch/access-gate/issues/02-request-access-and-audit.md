...
- [x] request_access 工具：申请触发原生审批；批准入账；拒绝返回原因；可列出当前生效授权
- [x] TTL 到期自动回落 ro，正在执行的命令跑完、新调用才回落
- [x] 可手动撤销，撤销立即生效
- [x] ssh 无授权 → 拒绝且错误信息指向 request_access；有授权 → 限时放行
- [x] subagent（独立 session id）不继承主会话授权
- [x] 审计日志逐条落 JSONL：授权、到期、撤销、rw 代发各一行，字段完整
- [x] 全仓测试绿，新增测试覆盖批准/拒绝/到期/撤销/ssh 拒绝路径
