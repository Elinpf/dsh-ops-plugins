...
集中凭证管理服务（`packages/ops-access-hub`，npm 名 `@elinpf/dsh-ops-access-hub`），core 的可选第二凭证来源。**决策见 `docs/adr/0005-access-hub.md` 与 `docs/adr/0009-named-tokens.md`，构建内容见 `docs/specs/0006-access-hub.md`**——这里只定义词汇：
...
- **具名 token (named token)**（ADR-0009）— 按人签发的一次性凭证：`name`（持有人标签）+ `role`（admin/read）+ 可选 `expiresAt`。**明文只在签发响应里出现一次**，落库只有 sha256 摘要与前 8 字符前缀；`POST /tokens` 签发、`GET /tokens` 看名册（不含摘要）、`DELETE /tokens/:id` 吊销（终态，立即 401）。未吊销记录里 `name` 唯一（走了可复用）。审计行记 `actor = name`，回答「是谁 resolve 的」；静态 token 不记 actor（共享凭据，不做假归属）。静态 token 刻意保留为 break-glass——吊销全部具名 admin token 也进得去。仍是 admin/read 两种角色，不是用户体系/RBAC。签发/吊销路径：Web UI 的 token 名册区块、`POST /tokens` API、CLI `dsh-ops-access-hub token create|list|revoke`（`--url` 在线或 `--data-dir` 离线，与 import 同纪律）
