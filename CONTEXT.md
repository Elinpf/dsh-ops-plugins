...
ops-tool-trace 通过它注册教义核心段和三条提醒规则（idle：5 步没更新 trace；nesting：step 全平挂 milestone 下；stale-step：开放 step 超 4 轮无定论——后半场的树腐烂，死路不登记，2026-09-10 实战靠用户两次催更才发现）；ops-prompts 自身不带业务内容。
...
- **提供方 ops-access/{k8s,ceph,ssh,prometheus}** — 每种凭证类型一个：该类型的 zod schema、字段加工（如 `~` 展开）、可选的 `validateContent` 保存时内容校验、可选的 **`knownLimits`**（ro 档凭据的已知盲区文案——k8s 写明 view ClusterRole 不含 nodes/PV/SC/VA/Secret 内容、ceph 写明 ro caps 跑不了 tell/fs-subvolume/rbd 管理命令；经 @ 引用注入和 `list_access help` 透出，agent 从文本学前凭据边界，而不是拿一次 403 换一次认知、compaction 后再重撞——2026-09-10 实战）。apply 里只调定义包的 `registerAccessProvider(ctx, provider)` 帮手注册——不要手写 `ctx.inject(['opsAccess'], ...)`（静态 inject 兄弟行服务会让 loader 死锁，帮手内含这段防护）。**ssh 提供方注册两个 kind**:`ssh`（主机条目）+ `ssh-cred`（可复用凭证条目），见下「引用字段」
- **引用字段 (references)** — core 的通用机制：provider 声明 `references: { 字段名: kind }`（ssh 声明 `cred → ssh-cred`），resolve 时把被引用条目的字段合并到引用方**之下**（同注册表、同 tier、只展开一层；引用方字段赢冲突，如 per-host 覆盖 user）。设计动机：一份密钥/密码登记一次、N 台主机共享，轮换只改一处——而不是每注册一台主机粘贴一遍私钥。broker 只对引用方咨询一次（凭证是主机的实现细节，不单独门控）；悬挂引用让引用方的 resolve 和 `canResolve` 预检一起失败并报出两端条目名。配套钩子 `validateResolved`：合并后校验，承载只在合并形状上成立的要求（ssh 的登录 user 可来自主机或凭证任一侧，schema 无法单独要求）。**hub 模式集成**：引用展开不走注册表直读——finalizeProfile 对被引用条目调同一个 `backend.loadTier(refKind, refName, tier, { materialize })`，被引用凭证的 fileField 内容在展开点上完成抓取+物化（resolve 物化、canResolve/list 只给路径不写盘），两个后端行为一致（hub-backend.spec.ts 的 ×5 引用用例钉住）
- **保存时内容校验 (validateContent)** — provider 可选钩子，文件类字段的粘贴内容**落盘前**做结构性校验（k8s：合法 YAML 且有 clusters/contexts/users，且 `current-context` 必须指向已定义的 context——ops 工具不带 `--context`，缺失或失效的 current-context 会让每次调用都炸；ceph：[global]/mon_host + keyring 的 key 行缩进且 base64 ≥16 字节；ssh：**盔甲检查 + 真实解析**——`ssh-keygen -y -P ''` 跑通才放行（与 ssh 连接时同一个解析器），加密私钥报 BatchMode 不可用的明确错误；密码字段必须单行——`sshpass -f` 只读文件首行，内部换行会让密码在使用时被静默截断）。钩子可以是**异步**的（ssh 要跑子进程）；provider 可声明 **`normalizeTrailingNewline`**（ssh/ceph 启用）在**校验前**把内容归一化为恰好一个结尾换行——PEM 的 END 行必须换行终止，粘贴/模型转述常丢这一字节（2026-08-27：ssh 密钥落盘缺尾换行 → 使用时 error in libcrypto，密钥数学关系完全自洽仍失败）。两条写入路径（admin UI、register_access）共用，校验失败 400/ok:false 且零文件 IO——粘贴丢格式（如 ceph keyring 的 TAB 缩进）在保存时就报明确错误，而不是使用时爆 cannot parse buffer
...
- **消费方 ops-tool-{kubectl,ceph,ssh,prometheus}** — 模型工具，按名字解析凭证后拼命令。留在 `packages/` 顶层：它们是工具层，不是凭证层。prometheus 是例外形态——它不走 ops-shell-tool 的命令工厂（目标是 HTTP API 不是 CLI），直接 fetch 该 profile 的 url + 可选 token 打 `/api/v1/query(_range)`

### ops-shell-tool

**命令工具工厂**（`packages/ops-shell-tool`，纯库不是插件）。所有消费方共享的那套机器的唯一事实源：标准结果形状 `{ exitCode, stdout, stderr, command, error? }`、output schema、render、execute 模板（`ctx.get('opsAccess')` 现解析 → buildCommand 拼命令 → `ctx.shell` 执行，30s 超时，信号死亡 exitCode 归一为 -1 **并在 error 字段注明首因，永不裸 -1**（超时杀：含秒数、「操作可能已部分生效」警告、远端显式超时建议；调用方中止：aborted 文案；其他信号杀：透传信号名，SIGKILL 指向 OOM/策略/人为终止——裸 -1 曾让模型把挂死端点误诊为「管道不稳定」，2026-08-27 实战），错误原样透传）。消费方只剩身份四件：工具名、解析的 kind、档案名参数名、`buildCommand`，外加可选的 `stderrNoise`——已知噪音 stderr 行的正则表（如 ceph 容器内缺默认 keyring 的告警），工厂在凭证擦除之后按行过滤，只丢精确匹配的行，其余 stderr 原样保留。`shellQuote` 是导出助手：**ssh 消费方把整条远端命令引成单一参数**（`&&`/`|`/`;`/`$()` 全在远端 shell 解释，本地零切割——2026-08-27 近失事件：未引用时 `&&` 后段会以 root 在本地执行，manifest 恢复链差点断在本地）；kubectl/ceph 维持原样拼接（本地管道过滤有用），且声明 `rejectShellComposition`——`;`/`&&`/`||`/反引号/`$()`/换行在执行前硬拒并给教学式报错（组合符后段会脱离前缀裸奔，报 `get: command not found` 诱使 agent 误诊集群，2026-09-10 跨 7 轮踩了 8+ 次；单个 `|` 管道仍允许），并开启 `perCallTimeout`——工具多出 `timeoutSec` 参数（1–600s，大池 `rados ls` 曾撞死 30s 固定上限）。**ceph 消费方按命令首词在 `[ceph, rbd, rados]` 白名单选二进制**（rbd/rados 是独立二进制不是 ceph 子命令——`ceph rbd ls` 的 no valid command found 曾诱使 agent 误诊为权限问题，2026-08-27），不包装的本地二进制（mount/ceph-fuse 等）直接拒绝并指路 ssh 工具；只读性由凭证 caps 在 mon/osd 强制执行，不靠二进制名兜底。
...
`resolve(kind, name)` 的返回：`{ kind, name, tier, description?, environment?, fields }`。`tier` 是本次实际发放的档位（broker 授权后为 `rw`，否则 `ro`），消费方用它标注凭证引用 token。`description`/`environment` 是所有类型通用的 envelope 字段；`fields` 是提供方 schema 校验+加工+**引用展开**后的类型特有字段（k8s 是 `kubeconfigPath`，ceph 是 `conf`/`keyring`/`name?`，ssh 是 `host`/`user`/`key?`/`password?`/`cred?`/`port?`——带 `cred` 时 `user`/`key`/`password` 已由 ssh-cred 条目并入，主机条目自身字段优先）。

**name 的 kind 前缀容忍（幂等）**：`resolve`/`canResolve` 接受 `ssh/b200-02` 这种带冗余 kind 前缀的写法（recall/@ 文案就是 `kind/name` 形态，agent 会整个传进来——2026-09-10 曾因此三连错）。注册名永远不含 `/`，剥离前缀无歧义。
...
- **面板授权 (panel grant)**（同上）— 人主动发起的授权，与申请批准的 grant **同构同寿命**（session 分键、TTL 有界、重启清空），只是跳过"agent 开口"。TTL 只能选档位（默认 10/30/60/120 分钟，Config 可配——曾经默认 5/10/30，60/120 的申请全被钳到 30，跨小时恢复被迫反复走全量审批，2026-09-10 实战），主动授权与审批共用同一份档位。待决申请超时未裁决的报错带行动指引（提示 agent 叫人看 /access-all 面板）——无人值守时不许静默失败
...
集中凭证管理服务（`packages/ops-access-hub`，npm 名 `@elinpf/dsh-ops-access-hub`），core 的可选第二凭证来源。**决策见 `docs/adr/0005-access-hub.md`、`docs/adr/0009-named-tokens.md` 与 `docs/adr/0010-token-admin.md`，构建内容见 `docs/specs/0006-access-hub.md`**——这里只定义词汇：
...
- **凭证平台 / access-hub** — 单一加密 JSON 文档存全部条目（AES-256-GCM，`<data-dir>/hub-data.json.enc`，写盘 tmp+rename 原子写）；master key 来自 env `ACCESS_HUB_KEY`（hex/base64）或 key 文件（首启自动生成，0600）。HTTP API 用 **Bearer token 认证**：静态 bootstrap token（admin 全量 / read 只读列表与解析，来自 flag/env，始终有效、不可吊销）+ 具名 token（见下）；timingSafeEqual 比较，401/403 区分。审计落 append-only JSONL（resolve/put/delete/token-*，只记成功操作，**永不记字段值**）。自带单文件中文 Web UI（token 存浏览器 localStorage）。v1 明文 HTTP，默认只绑 loopback，远程部署套 TLS 反代。**hub 是单点——数据文件与 master key 都要备份**
- **具名 token (named token)**（ADR-0009，精细管理见 ADR-0010）— 按人签发的一次性凭证：`name`（持有人标签）+ `role`（admin/read）+ 可选 `expiresAt`。**明文只在签发响应里出现一次**，落库只有 sha256 摘要与前 8 字符前缀；`POST /tokens` 签发、`GET /tokens` 看名册（不含摘要）、`PATCH /tokens/:id` 就地改标签/角色/有效期（`expiresAt: null` = 清除，**秘密不变**，角色下一个请求即生效）、`DELETE /tokens/:id` 吊销（终态，立即 401）。未吊销记录里 `name` 唯一（走了可复用）。名册状态四分：`active` / `expiring`（≤7 天）/ `expired` / `revoked`，判定收敛在 `tokens.ts` 的 `tokenStatus()`，CLI 与 Web UI 共用。审计行记 `actor = name`，回答「是谁 resolve 的」，`token-update` 行另记 `changes`（被改字段名，非值）；静态 token 不记 actor（共享凭据，不做假归属）。静态 token 刻意保留为 break-glass——吊销全部具名 admin token 也进得去。仍是 admin/read 两种角色，不是用户体系/RBAC。管理路径：Web UI 的 token 名册区块（签发/一次性明文/搜索筛选/就地编辑/吊销）、`/tokens` API、CLI `dsh-ops-access-hub token create|list|update|revoke`（`--url` 在线或 `--data-dir` 离线，与 import 同纪律）
...
- **物化 (materialize)** — hub 模式下 resolve 时把 provider `fileFields` 的内容落盘到 **`hubCacheDir`**（默认 `~/.dsh-ops/hub-cache`，**独立于** yaml 的 credentialsDir）下的 `kind/name/tier/field`（0600、原子写、内容相同跳过），fields 里的值替换为该本地路径——下游 provider schema、凭证引用 token、CLI 看到的 profile 与 yaml 来源同形，路径仍不出本机。`materialize: false` 用于 canResolve/list/getEntry 等元数据读（如门批准前的预检）：只替换将然路径、**不落盘**，元数据读不写秘密文件。写入方向相反：putTier 把本地受管文件的内容读出上传（envelope 合并语义与 yaml 一致：undefined 保留、`''` 删除）。**TTL 缓存纪律**（2026-09-08）：hub 模式下本地凭证文件——物化的读缓存和写入路径的暂存——一律 TTL 约束、不是永久副本（ro/rw 同律，也压「别人拷文件」的窗口）：启动全量清扫缓存目录（grant 账本随重启清空，rw 材料不得比账本活得久）、`materializeTtlMinutes`（默认 15）定时过期、resolve 现取现物化到期透明重建；清扫器只走 hubCacheDir，yaml 回退文件永不被碰
...
- **rw 注册申请 (registration request)** — ADR-0008：agent 用 `register_access` 传 `tier: "rw"` 提交申请（提交前走与真实写入相同的 validateContent + zod 校验），排队在 hub 的加密文档里（`/requests` API，`request/approve/reject` 审计动作）；管理员在 dsh 凭证管理设置区审查字段内容后批准/拒绝（core 代理路由 `/ops-access/admin/requests*`，浏览器不持 hub token）；批准后 hub 才写入该 tier；**决定即擦除申请里的 fields**。yaml 模式无此通道（rw 仍只能人写）
