...
Nineteen of the 21 packages under `packages/` share one version number (changesets `fixed` group) and publish together as a lockstep suite ("插件集 vX"). The two exceptions: `ops-access-hub` — not a dsh plugin, independently versioned (see Packages) — and `test-support` — private, never published (shared spec helpers). npm names are `@elinpf/dsh-ops-*`, plus the single deployment package `@elinpf/dsh-ops`.
...
- `ops-access-hub/` (`@elinpf/dsh-ops-access-hub`) — **the repo's one exception package: not a dsh plugin.** A standalone, separately deployable credential hub: AES-256-GCM encrypted-at-rest single-document store, Bearer-token HTTP API with an append-only audit log, a single-file web UI, and a YAML registry importer (`dsh-ops-access-hub serve` / `import` / `token`). Auth is two static bootstrap tokens (admin/read, flag/env, always valid, the break-glass path) plus **named tokens** (ADR-0009): per-person issue with a role and optional expiry, stored only as a SHA-256 digest, individually revocable, recorded as the audit `actor`, and editable in place — label / role / expiry, secret untouched (ADR-0010). It also hosts the troubleshooting knowledge base (`/cases` routes — deliberately writable by the read role, cases hold no secrets) consumed by `ops-knowledge`. `ops-access/core` can use it as its credential source (Config `source: 'hub'`, via the internal `AccessBackend` seam — `YamlBackend` default, `HubBackend` fetches per call and materializes file-field content to local 0600 files). No `cordis.patch.yml`, no `dsh` field in package.json, never mounted in a preset, outside the changesets `fixed` group (own version cadence). Design: `docs/adr/0005-access-hub.md` + `docs/adr/0009-named-tokens.md` + `docs/adr/0010-token-admin.md`; contract: `docs/specs/0006-access-hub.md`.
...
- `ops-knowledge` — troubleshooting knowledge base (preset plane, `ops-orchestration` realm): `knowledge_search` / `knowledge_record` / `knowledge_hit` tools backed by the hub's `/cases` routes, plus a session-start case-index reminder through `ops-prompts`. No config + no `ACCESS_HUB_URL`/`ACCESS_HUB_READ_TOKEN` env → the plugin no-ops; hub outages surface as tool error results, never throws.
...
- The access-gate (`packages/ops-access/gate`, credential brokering, ro/rw tiers, per-session grants, audit log) is **implemented and wired into the ops preset** (inside the `ops-access-registry` realm, alongside its `opsAccessGate` isolate symbol); see `docs/adr/0001-access-gate.md` and `docs/specs/0001-access-gate.md` before touching authorization. Its threat model is "prevent mistakes, not malice" — same-UID in-process secrecy is explicitly out of scope. Every consumer tool also carries an optional per-call `tier: 'ro'|'rw'` parameter: `'ro'` is a deliberate downgrade under an rw grant (grant untouched, no rw-issue audit), `'rw'` demands the write tier and denies loudly (request_access guidance) when ungranted; core's `resolve` takes it as the 4th-arg `AccessRequest` and the gate's broker owns the policy.
