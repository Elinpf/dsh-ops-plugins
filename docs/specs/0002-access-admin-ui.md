...
ops-access 的凭证档案目前全靠手编 YAML 文件（access.yaml 和 access-rw.yaml）。手编的痛点是：填错了字段只有 agent 真正尝试 resolve 时才炸（或者申请授权被门拒绝时才知道 rw 条目不存在）；rw 文件对人不可见、没有校验入口；加一个新 kind 的条目要去翻 provider 的 fieldsDoc 文档才知道要填什么。运维工程师需要一个浏览器界面：录入凭证条目、选 ro 或 rw 档位、即时校验"这个条目真的能解析"，不用手编 YAML 不用重启。
...
安全纪律：**只写不回读**——表单永远是空表单，填完提交覆盖写，不回填旧值。保存后页面回到列表视图，只显示条目的 envelope（kind/name/description/environment）和验证状态，fields（路径、连接参数）从屏幕消失。rw 的 fields 永远不流向浏览器。kind 列表和 provider 的字段结构通过 zod v4 的 z.toJSONSchema() 序列化为标准 JSON Schema 发到前端，动态渲染表单字段。
...
4. 作为运维工程师，我希望在 UI 里新增一个凭证条目，选好 kind、填好字段、选 ro 或 rw 档位，提交后条目就写进文件，这样不用手编 YAML。
...
- POST /ops-access/admin/entry — body: { kind, name, tier, fields, description?, environment? }，调 writeEntry()，返回 { ok: boolean, error?: string }
...
- 列表视图：表格列出所有 AdminEntry（kind/name/description/environment + ro/rw 验证图标），刷新按钮，新增按钮
- 表单视图：kind 下拉（从 /admin/kinds 拉取）、profile name 输入、tier 选择（ro/rw 单选）、根据 kind 的 JSON Schema 动态渲染的字段输入框、description/environment 可选输入、提交按钮
...
- **writeEntry**：upsert 新条目 → 文件写入 → resolve 能拿到 → 字段正确；upsert 覆盖已有条目 → 旧值被覆盖；schema 校验失败 → 不写文件 + 抛错；rw 文件不存在 → 创建文件
- **deleteEntry**：删除已有条目 → 文件更新 → resolve 抛 "not found"；删除不存在的条目 → 返回 false；文件不存在 → 返回 false
- **listAll**：ro 有 rw 没有 → 该条目 ro.ok=true rw.ok=false；schema 校验失败的条目 → ok=false + error 有原因；两个文件都有同名条目 → 合并为一行；envelope 字段正确传递；响应不含 fields
