...
| 常驻策略（信任环境长期放行） | 绕开"人一次性批准"原则，需独立威胁模型讨论——缓议（**已决定 2026-08-28，票 11 → ADR-0006**：威胁模型讨论完成，暂不实施；重提条件与所需证据在 ADR-0006 末节） |
| 封禁（连 ro 都锁） | broker 需新增第四态，等面板落地后看真需求——缓议（**已落地 2026-08-28，票 12**：deny 为进程级、持久化到 `~/.dsh-ops/denied.json`——事故冻结重启即失效等于没冻结；封禁连带收回所有会话的该档案授权；面板档案行红点（预留的 `.ops-access-panel-dot.deny`）+ 封禁/解封两段确认；审计事件 deny/undeny/deny-block） |
| 跨会话总览与撤销 | 面板是会话作用域 UI，硬塞跨会话列表语义混乱——缓议（**已落地 2026-08-28，票 13**：独立面板 `/access-all` 而非塞进会话面板，语义混乱问题用「另一个 surface」解法绕过；决策 8 反向检查点通过——注册 API 原样够第二消费方用，未泛化） |
