...
**（2026-08-28 补记，实战实测，票 08）** 威胁模型的边界事实在实战中显形，三条明记：

1. **bash 绕过是设计内边界，且真的会发生**：持通用 bash 的 agent 永远能绕过凭证门（读宿主机密钥、自跑 ssh/kubectl）。2026-08-27 实测中，门控 ssh 工具故障期间 agent 自行改用 bash + `~/.ssh/id_rsa` 完成了全部节点级操作（含高风险动作）——面板批的 ssh grant 没承载真实流量，**授权体感与真实执行脱钩**：操作员以为自己在管授权，实际操作发生在未授权通道。
2. **审计完整性声明限定为「经门控工具的操作」**：门控通道的 grant/gated-issue 逐条进 ops 审计日志；bash 绕道的同等操作只进 session 事件流。审计声明按此限定，不声称覆盖进程内一切动作。
3. **主要缓解是把门控工具修可靠，不是堵 bash**：实测中的绕道几乎都是工具失败逼出来的（票 04 的远端命令引号、票 05 的密钥校验缺陷），不是 agent 想逃避授权——工具可靠则绕道无动机。这也是 B/C（sandbox 收敛 bash、bash 内容审计）明确不做的现实依据。
...
| sandbox 收敛通用 bash（堵绕过） | 建设成本高一个数量级且损害排查自由度（决策 1）；实测绕道由工具失败逼出，修工具才是对症下药（票 08） |
| bash 命令内容审计（补审计缺口） | 自由命令字符串的语义解析不可可靠（决策 3 同理）；绕道操作在 session 事件流已有完整记录，ops 审计声明限定为门控通道即可（票 08） |
