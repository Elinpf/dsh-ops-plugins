...
- 日期：2026-09-08
- 状态：已决定（explore 线已实现并实测，待合回 master）
...
- **access-hub 合并注意**：hub 后端在 resolve 时物化 fileField 内容；引用展开必须先于物化完成，否则主机条目里没有 fileField 可供物化。已记入 CONTEXT.md。
- UI 零改动：access-ui 的 kind 列表来自 `listKinds()`,ssh-cred 自动出现。
- 实测（.dsh-explore,2026-09-08)：密钥路径——一份 ssh-cred 两台主机真实连通；密码路径——本机专用 sshd(:2299)+ 测试用户,sshpass 登录成功;悬挂引用——resolve 报错指路 + admin list 标 not-ok。测试条目已清理。
