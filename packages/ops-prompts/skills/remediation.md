---
name: remediation
description: 调查收口后需要 rw 修复时使用的四阶段纪律——出方案/验证/人审/执行；验证与执行必须 spawn 干净上下文的子 agent，方案必须五要素齐全。只读排查不要用。
whenToUse: trace 调查树收口、需要 rw 权限做修复变更时
---
...
- 启动：trace 调查收口（goal 有定论），且修复需要 rw 权限
